...
export default function CreateBlog() {
  ...
  const [desc, setDesc] = useState(""); // maps to `content` in backend
  ...
  const handlePublish = async (e) => {
    ...
    try {
      ...
      fd.append("content", desc); // backend expects `content`
      fd.append("category", category);
      if (bannerFile) fd.append("banner", bannerFile); // multer field name: banner
        ...
        >
          ...
          <div className="mt-6">
            ...
            <input
              ...
              className="w-full rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-sm text-white placeholder-white/40 outline-none focus:border-indigo-300/60 focus:bg-white/15"/>
          </div>
          ...
          {/* Category dropdown */}
            <div className="md:pt-2">
              ...
              <select
                ...
                className="w-full rounded-xl border border-white/10 bg-white/10 px-3 py-2 text-sm text-white placeholder-white/40 outline-none focus:border-indigo-300/60 focus:bg-white/15"
              >
                {CATEGORIES.map((c) => (
          ...
          <div className="mt-6 flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={!canPublish}
                className="group relative flex items-center justify-center gap-2 overflow-hidden rounded-xl bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-cyan-400 px-6 py-2.5 text-sm font-semibold text-white shadow-lg shadow-indigo-900/30 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
              >
                {loading ? (
                  <>
